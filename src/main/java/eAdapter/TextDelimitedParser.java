...
public class TextDelimitedParser implements Parser {
	...
	public Document parseDocument(ParserArguments parserArgs) {
		...
		String line = getLine(parserArgs.getReader(), parserArgs.getDelimiters().getNewRecord());
		...
		if (delimiters.getTextQualifier() == NULL_CHAR) {
			...
			for (int pos = 0; pos < line.length(); pos++) {
				...
				if (line.charAt(pos) == delimiters.getFieldSeparator()) {
					...
					pos = parseQualifiedData(line, pos, valueBuilder, parserArgs);
				}
	...
	private String getLineEndingWithNewLine(BufferedReader reader, char newLine) {
		String line = null;
	...
	private String getLineEndingWithoutNewLine(BufferedReader reader, char newLine) {
		StringBuilder line = new StringBuilder();
		char[] buffer = new char[1];
		
		try {
			// read in a line from the reader
			while (reader.read(buffer, 0, 1) != -1 && buffer[0] != newLine) {
	...
	private String getLine(BufferedReader reader, char newLine) {
		// determine which method is used to read in a line depending on the new record delimiter
		if (newLine == NEW_LINE) {
			return getLineEndingWithNewLine(reader, newLine);
		}
		else {
			return getLineEndingWithoutNewLine(reader, newLine);
		}
	}
			
	private int parseQualifiedData(String line, int pos, StringBuilder valueBuilder, ParserArguments parserArgs) {
		Delimiters delimiters = parserArgs.getDelimiters();
		...
		for (pos = pos + 1; pos < line.length(); pos++) {
			...
			if (pos == line.length() - 1 && line.charAt(pos) != delimiters.getTextQualifier()) {
				...
				pos = 0;
				line = getLine(parserArgs.getReader(), delimiters.getNewRecord());
			}
